...
function validateHelperRequest(req, res, next) {
    const { pickupZip, dropoffZip, rooms, helpers, volume, date } = req.body || {};
    const errors = [];
    ...
    if (!isZip(pickupZip))
        errors.push("pickupZip must be 5 digits (US ZIP).");
    if (!isZip(dropoffZip))
        errors.push("dropoffZip must be 5 digits (US ZIP).");
