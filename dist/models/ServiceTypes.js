...
export class LoadersService {
    ...
    validate() {
        ...
        if (this.hourly_rate_per_person < 0)
            errors.push('Hourly rate must be positive');
        if (this.minimum_hours < 1)
            errors.push('Minimum hours must be at least 1');
        return errors;
    }
}
export class DriversService {
    ...
    validate() {
        ...
        if (this.vehicle_types.length === 0)
            errors.push('At least one vehicle type required');
        if (this.per_mile_rate && this.per_mile_rate < 0)
            errors.push('Per mile rate must be positive');
...
export class TruckRentalService {
    ...
    validate() {
        ...
        if (!this.provider_id)
            errors.push('Provider ID is required');
        if (this.truck_types.length === 0)
            errors.push('At least one truck type required');
        if (this.daily_rate && this.daily_rate < 0)
            errors.push('Daily rate must be positive');
        if (this.min_rental_days < 1)
            errors.push('Minimum rental days must be at least 1');
...
export class ContainerService {
    ...
    validate() {
        ...
        if (!this.provider_id)
            errors.push('Provider ID is required');
        if (this.container_sizes.length === 0)
            errors.push('At least one container size required');
