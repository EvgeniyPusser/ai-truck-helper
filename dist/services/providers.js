...
function selectTruckSmart(rooms, volume, weight) {
    ...
    return (trucks.find((t) => v <= t.volume_m3 && weight <= t.max_weight_kg) ||
...
function calculateDistance(pickupZip, dropoffZip) {
    ...
    let distance;
    if (zipDiff < 100)
        distance = 5 + Math.random() * 15; // 5-20 miles for nearby zips
    else if (zipDiff < 1000)
        distance = 20 + Math.random() * 80; // 20-100 miles
    else if (zipDiff < 5000)
        distance = 100 + Math.random() * 200; // 100-300 miles
    else
        distance = 300 + Math.random() * 1000; // 300-1300 miles for cross-country
...
export async function findHelpers(params) {
    ...
    const weekend = ["Sat", "Sun"].includes(moveDate.toLocaleString("en-US", { weekday: "short" }));
    if (weekend)
        baseHourlyRate = Math.round(baseHourlyRate * 1.15);
    ...
    if (isMajorCity)
        baseHourlyRate = Math.round(baseHourlyRate * 1.2);
    ...
    switch (truck.id) {
        case 'van':
            truckDailyCost = 29.95;
            break;
        case 'sprinter':
            truckDailyCost = 39.95;
            break;
        case 'small_box':
            truckDailyCost = 49.95;
            break;
        case 'medium_box':
            truckDailyCost = 69.95;
            break;
        case 'large_box':
            truckDailyCost = 89.95;
            break;
