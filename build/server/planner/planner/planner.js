...
const trucks = JSON.parse(fs.readFileSync(path.join(dataDir, "trucks.json"), "utf-8"));
const helpers = JSON.parse(fs.readFileSync(path.join(dataDir, "helpers.json"), "utf-8"));
...
function estimateDistance(from, to) {
    ...
    if (from === "Los Angeles, CA" && to === "Phoenix, AZ")
        return 600;
    if (from === "Los Angeles, CA" && to === "San Diego, CA")
        return 130;
    return 100; // default
}
// Calculate price function
function calculatePrice(distance, truck, volume, helpersCount) {
...
function selectPlan(clientRequest) {
    ...
    const availableTrucks = trucks.filter((t) => t.location === from &&
    ...
    availableTrucks.sort((a, b) => b.rating - a.rating || a.pricePerMile - b.pricePerMile);
    const chosenTruck = availableTrucks[0];
    // Helpers selection
    let helpersSelected = [];
    if (needHelpers) {
        // Filter helpers by location and availability
        const availableHelpers = helpers.filter((h) => h.location === from && h.availableDates.includes(date));
        // Choose helpers with best rating, max 2 helpers
        availableHelpers.sort((a, b) => b.rating - a.rating);
        helpersSelected = availableHelpers.slice(0, 2);
    }
    // Calculate total price
    const totalPrice = calculatePrice(distance, chosenTruck, volume, helpersSelected.length);
    // Return plan summary
    return {
        clientRequest,
        chosenTruck,
