...
import {
  saveHelperQuoteRequest,
  updateHelperQuoteAnalysis,
} from "../services/helperQuoteRequests.js";
...
async function analyzeAndSaveHelperRequest(id, request, result) {
  if (!id || !isMongoConfigured()) return;
  ...
  try {
    ...
    await db.collection("helper_requests").updateOne(
      { _id: id },
      {
        $set: {
          aiAnalysis: {
